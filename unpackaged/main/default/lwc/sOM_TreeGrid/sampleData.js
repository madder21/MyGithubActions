...
import getAccountList from "@salesforce/apex/STWController.getSTWList";
    ...
    if (data) {
      ...
      for (let i = 0; i < accounts.length; i++) {
        let jsonData =
          '{"Id":"' +
          parentId +
          '","country__c":"","storeOrigin__c":"' +
          accounts[i].storeOrigin__c +
          '","postalCode__c":""}';
        if (parentId === 0) {
          parseData.push(JSON.parse(jsonData));
          parseData[parentId]._children = [];
          parseData[parentId]._children.push(accounts[i]);
          parentEntity = accounts[i].storeOrigin__c;
          parentId++;
        } else {
          if (accounts[i].storeOrigin__c === parentEntity) {
